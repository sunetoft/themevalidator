...
export default function ThemeDetailClient({
  ...
  const [syncingGraphFor, setSyncingGraphFor] = useState<string | null>(null)
  ...
  const syncToGraph = async (thesisId: string) => {
    setSyncingGraphFor(thesisId)
    try {
        ...
        setDetail(prev => {
          ...
          return {
            ...
            theses: prev.theses.map(t =>
              t.id === thesisId ? { ...t, graphSyncedAt: new Date().toISOString() } : t
            ),
          }
        })
      } else {
  ...
  const handleAddTicker = async (thesisId: string) => {
    const ticker = (tickerInputs[thesisId] ?? '').trim().toUpperCase()
    if (!ticker) {
      toast.error('Please enter a stock ticker')
      return
    }
    setAddingTickerFor(thesisId)
    ...
    try {
      ...
      setTickerInputs(prev => ({ ...prev, [thesisId]: '' }))
      // Re-fetch theme detail to get updated analysis
      fetch(`/api/themes/${themeId}`)
        .then(r => r.json())
                      ...
                      >
                        <div className="px-5 pb-5 border-t border-border/40">
                          ...
                          <div className="mt-4 pt-4 border-t border-border/40 space-y-4">
                            ...
                            {isAdmin && (
                              <div>
                                <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">Add Stock to Basket</h3>
                                <div className="flex gap-2">
                                  <input
                                    type="text"
                                    inputMode="text"
                                    value={tickerInputs[thesis.id] ?? ''}
                                    onChange={(e) => setTickerInputs(prev => ({ ...prev, [thesis.id]: e.target.value.toUpperCase() }))}
                                    onKeyDown={(e) => { if (e.key === 'Enter' && addingTickerFor !== thesis.id) handleAddTicker(thesis.id) }}
                                    placeholder="e.g. NVDA"
                                    disabled={addingTickerFor === thesis.id}
                                    className="flex-1 px-3 py-2 bg-muted/40 border border-border rounded-lg text-sm font-mono focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary/50 transition-all disabled:opacity-50 uppercase placeholder:normal-case"
                                  />
                                  <button
                                    onClick={() => handleAddTicker(thesis.id)}
                                    disabled={addingTickerFor === thesis.id || !(tickerInputs[thesis.id] ?? '').trim()}
                                    className="px-4 py-2 bg-primary hover:bg-primary text-white font-medium text-sm rounded-lg transition-all flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed shadow-sm shadow-primary/20"
                                  >
                                    {addingTickerFor === thesis.id ? (
                                ...
                                {addingTickerFor === thesis.id && tickerProgress && (
                            ...
                            {/* Create Trading Strategy — paying users + admin only */}
